...
const MatchingEngine = (function () {
  ...
  function extractCustomer(row) {
    return {
      company: pick(row, ['업체명', 'companyName']),
      industry: pick(row, ['업종', 'industry']),
      years: pick(row, ['업력', 'businessYears']),
      revenue: pick(row, ['연매출', '연매출규모', 'revenue', '매출']),
      employees: pick(row, ['종업원수', 'employees']),
      fundType: pick(row, ['자금유형', '필요한자금유형', 'fundType', '자금용도']),
      desiredLoan: pick(row, ['희망대출금액', 'desiredLoanAmount', '희망자금']),
      existingLoan: pick(row, ['기존대출규모', 'existingLoanAmount', '기존대출']),
      certifications: pick(row, ['보유인증', 'certifications']),
      credit: pick(row, ['신용상태', 'creditStatus']),
      region: pick(row, ['지역', 'region']),
      bizType: pick(row, ['사업자유형', 'bizType']),
      concerns: pick(row, ['애로사항', 'concerns', '현재애로사항']),
      receiptNo: pick(row, ['접수번호', 'receiptNo']),
      receivedAt: pick(row, ['접수일', 'submittedAt']),
      status: pick(row, ['상태'])
    };
  }
